...
class VideoFolderDataset(torch.utils.data.Dataset):
    def __init__(self, folder, counter = None, cache=None, min_len=4, train = True):
        ...
        self.total_frames = 0
        self.images = []
        self.labels = np.load(folder + 'labels.npy',allow_pickle=True, encoding='latin1').item()
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        path = self.dir_path + self.dataset[item][0].decode('UTF-8')
        id = self.dataset[item][0].decode('UTF-8').replace('.png','')
        im = PIL.Image.open(path)
        im = im.convert('RGB')
