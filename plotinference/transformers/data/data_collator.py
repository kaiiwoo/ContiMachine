...
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling):
    ...
    def _whole_word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        for (i, token) in enumerate(input_tokens):
            ...
            # 하드코딩..
            if token in ['SPACE','COMPUTERS', 'MILITARY', 'NEGATIVE', 'POLITICS', 'POSITIVE', 'RELIGION', 'SCIENCE', 'CLICKBAIT' ,'LEGAL']:
                continue
        ...
        num_to_predict = random.randint(1, min(max_predictions, max(1, int(round(len(input_tokens) * self.mlm_probability)))))
